  ...
  getTeamData(teamList) {
    ...
    teamRef.get().then(function (doc) {
      if (doc.exists) {
        ...
        //console.log("stateTeam", this.state.teams );
      }
    }).then(result => {this.setState({ teams: result }), console.log("statet", this.state.teams)})
  }
  onEnterEvent() {
    // e.preventDefault();
      ...
      judgeRef.get().then(function (doc) {
        ...
        if (result == "No such event") {
          ...
          if (result[0] == "You are not in this event") {
            ...
            //this.getTeamData(result);
            //console.log("scorel", scoreList)
            //this.setState({ teams: result })
            var sl = this.getTeamData(result);
